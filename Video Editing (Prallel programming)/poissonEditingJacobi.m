function [Iedit, tsoln] = poissonEditingJacobi (Itgt, Isrc, Msrc, ijTopLft, ...
...
%     end
    % convert images into gpuArray
%     gIedit = gpuArray(Iedit);
%     ghLaplace = gpuArray(hLaplace);
%     gMedit = gpuArray(Medit);
%     gB = gpuArray(B);
%     wait(gpuDev);
    % start timing
    tic
    ...
    for k = 1 : nMaxIter
        ...
        ILaplaceNbr  = imfilter( Iedit, ghLaplace, bndFilter );
